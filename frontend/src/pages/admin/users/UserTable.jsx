...
// Tabla de usuarios reutilizable: la pestaña "Usuarios" la usa sin restricción,
// y la vista de detalle de un grupo la usa fijada a ese grupo (fixedGroup).
export default function UserTable({ fixedGroup, showGroupFilter = true }) {
  ...
  function loadUsers() {
    ...
    const groupParam = fixedGroup || filterGroup;
  ...
  async function bulkArchive() {
    ...
    if (!confirm(`¿Archivar ${selected.size} usuario(s)?`)) return;
  ...
  function downloadCSV() {
    const header = ['Usuario', 'Correo', 'PPP', 'Riesgo', 'Grupos', 'Se unió el', 'Último inicio de sesión', 'Estado'];
    const lines = users.map(u => [
      u.display_name,
      u.email,
      u.ppp === null || u.ppp === undefined ? '' : `${u.ppp}%`,
      u.risk_score || 0,
      [u.org_unit_name, u.custom_groups_csv].filter(Boolean).join(', '),
      fmtDate(u.created_at),
      fmtDate(u.last_login_at),
      u.status === 'active' ? 'Activo' : 'Archivado',
    ]);
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table className="w-full text-sm">
          <thead style={{ backgroundColor: '#001B71' }} className="text-white">
            <tr>
              <th className="p-3 w-8"><input type="checkbox" checked={users.length > 0 && selected.size === users.length} onChange={toggleSelectAll} className="w-4 h-4 rounded" /></th>
          ...
          <tbody>
            {users.map(u => {
              ...
              return (
                <tr key={u.id} className="border-t border-gray-50 hover:bg-gray-50">
                  <td className="p-3"><input type="checkbox" checked={selected.has(u.id)} onChange={() => toggleSelect(u.id)} className="w-4 h-4 rounded" /></td>
                  <td className="p-3">
            ...
            {!loading && users.length === 0 && <tr><td colSpan="8" className="p-8 text-center text-gray-400">No se encontraron usuarios</td></tr>}
            {loading && <tr><td colSpan="8" className="p-8 text-center text-gray-400">Cargando...</td></tr>}
          </tbody>
        </table>
      </div>
